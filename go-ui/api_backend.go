...
func setupRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("/api/sandboxes/spoof_whitelist", func(w http.ResponseWriter, r *http.Request) {
		switch r.Method {
		case "GET":
			...
			procs := sandboxManager.GetSpoofWhitelist(boxName)
			if procs == nil {
				procs = []string{}
			}
			jsonResponse(w, map[string]interface{}{"processes": procs})
		case "POST":
			data := parseJSONBody(r)
			boxName, _ := data["box_name"].(string)
			if boxName == "" {
			...
			procsRaw, _ := data["processes"].([]interface{})
			var procs []string
			for _, p := range procsRaw {
				if s, ok := p.(string); ok && strings.TrimSpace(s) != "" {
					procs = append(procs, strings.TrimSpace(s))
				}
			}
			sandboxManager.SetSpoofWhitelist(boxName, procs)
			jsonResponse(w, map[string]interface{}{"status": "ok", "applied": len(procs)})
		default:
	...
	mux.HandleFunc("/api/config", func(w http.ResponseWriter, r *http.Request) {
		...
		case "GET":
			jsonResponse(w, map[string]interface{}{
				...
				"spoof_whitelist_processes":   config.Get("spoof_whitelist_processes"),
			})
		case "POST":
			data := parseJSONBody(r)
			...
			if rawProcs, ok := data["spoof_whitelist_processes"].([]interface{}); ok {
				config.Set("spoof_whitelist_processes", rawProcs)
			}

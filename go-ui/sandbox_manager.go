...
// spoofWhitelistKeys are the 4 keys used to whitelist a process from HWID spoofing.
var spoofWhitelistKeys = []string{
	"HideFirmwareInfo",
	"RandomRegUID",
...
func (sm *SandboxManager) ToggleSpoof(boxName string, enable bool) {
	...
	if enable {
		...
		// Auto-apply the global whitelist processes from config
		if sm.config != nil {
			procs := sm.config.GetSpoofWhitelistProcesses()
			sm.SetSpoofWhitelist(boxName, procs)
		}
	} else {
		for _, tpl := range spoofTemplates {
		...
		// Clear whitelist entries too
		sm.SetSpoofWhitelist(boxName, nil)
	}
}


// GetSpoofWhitelist reads the Sandboxie.ini for the given box and returns the
// list of process names that have been whitelisted (i.e. have "key=proc,n" entries).
func (sm *SandboxManager) GetSpoofWhitelist(boxName string) []string {
	iniPath := `C:\Windows\Sandboxie.ini`
	if sm.config != nil {
		if p := sm.config.GetPath("sandboxie_ini"); p != "" {
			iniPath = p
	...
	if len(sections)-1 == len(matches) {
		for i, match := range matches {
			if strings.EqualFold(match[1], boxName) {
				sectionContent := sections[i+1]
				// Find all lines like: HideFirmwareInfo=someproc.exe,n
				re := regexp.MustCompile(`(?mi)^(?:HideFirmwareInfo|RandomRegUID|HideDiskSerialNumber|HideNetworkAdapterMAC)=([^,\r\n]+),n`)
				for _, sub := range re.FindAllStringSubmatch(sectionContent, -1) {
					proc := strings.TrimSpace(sub[1])
					if proc != "" && !seen[proc] {
						seen[proc] = true
...
// SetSpoofWhitelist replaces the whitelist entries for the given box.
// It first removes all existing whitelist entries for the 4 keys, then
// adds fresh entries for each process in the provided list.
func (sm *SandboxManager) SetSpoofWhitelist(boxName string, processes []string) {
	runCmd := func(args ...string) {
		cmd := exec.Command(sm.sbieIniExe, args...)
		cmd.SysProcAttr = &syscall.SysProcAttr{HideWindow: true}
		cmd.Run()
	...
	// Clear all existing whitelist entries for the 4 keys
	for _, key := range spoofWhitelistKeys {
		runCmd("delete", boxName, key+",n")
	}
	...
	for _, proc := range processes {
		...
		for _, key := range spoofWhitelistKeys {
			// SbieIni.exe append <box> <key> <value>
			// The ",n" is the value suffix Sandboxie expects for whitelisting
			runCmd("append", boxName, key, proc+",n")
		}
	}
}
